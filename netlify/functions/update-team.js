...
exports.handler = async (event) => {
  ...
  if (!actor || actor.role !== 'admin') {
    return json(403, { error: 'Alleen admins mogen teams bewerken.' });
  }
  ...
  if (existingError) {
    return json(500, { error: `Kon team niet controleren: ${existingError.message}` });
  }

  if (!existing) {
    return json(404, { error: `Team met id ${teamId} bestaat niet.` });
  }
  ...
  if (updateError) {
    return json(500, { error: `Team bijwerken mislukt: ${updateError.message}` });
  }
