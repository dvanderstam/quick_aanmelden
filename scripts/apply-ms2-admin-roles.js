...
async function main() {
  ...
  const teamAdminUsernames = ['giovani.chirinos', 'randolf.axwijk'];
  ...
  if (updateError) {
    throw new Error(`Could not set MS-2 teamAdmins: ${updateError.message}`);
  }
  ...
  if (error) {
    throw new Error(`Could not verify MS-2 teamAdmins: ${error.message}`);
  }

  console.log('MS-2 admin role result:');
